...
export const signUp = createServerFn({ method: "POST" })
  .inputValidator(
    (input: { email: string; password: string; metadata?: Record<string, unknown> }) => input,
  )
  .handler(
    async ({
      data,
    }): Promise<{ ok: true; user: AuthUser } | { ok: false; code: string; message: string }> => {
      const { createUser, requestEmailConfirmation, AuthError } =
        await import("@/lib/auth/users.server");
      ...
      try {
        ...
        await requestEmailConfirmation(user.id, user.email, originFromRequest()).catch(
          () => undefined,
        );
...
export const signOut = createServerFn({ method: "POST" }).handler(async () => {
  const { revokeSession, readCookie, SESSION_COOKIE, buildClearCookie } =
...
export const verifyEmailCode = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({ data }): Promise<{ ok: true; user: AuthUser } | { ok: false; message: string }> => {
      const users = await import("@/lib/auth/users.server");
      const { buildSessionCookie } = await import("@/lib/auth/session.server");
      const { toAuthUser } = await import("@/lib/auth/serialize.server");
      try {
...
export const verifyAuthToken = createServerFn({ method: "POST" })
  .inputValidator(
    (input: { token: string; type: "magic_link" | "password_reset" | "email_confirm" }) => input,
  )
  .handler(
    async ({ data }): Promise<{ ok: true; user: AuthUser } | { ok: false; message: string }> => {
      const users = await import("@/lib/auth/users.server");
...
export const updateAuthUser = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (input: { metadata?: Record<string, unknown>; password?: string; email?: string }) => input,
  )
  .handler(
    async ({ data, context }): Promise<{ ok: boolean; user?: AuthUser; message?: string }> => {
      const users = await import("@/lib/auth/users.server");
      const { toAuthUser } = await import("@/lib/auth/serialize.server");
      try {
...
export const revokeOtherSessions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const { revokeAllSessions, readCookie, SESSION_COOKIE } =
      await import("@/lib/auth/session.server");
