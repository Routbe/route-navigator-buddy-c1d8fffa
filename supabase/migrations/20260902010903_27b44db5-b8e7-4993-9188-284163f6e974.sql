...
create or replace view public.profile_handles
with (security_invoker = true) as
select username
from public.profiles
where username is not null;
...
update public.profiles set url_style = 'u_at' where url_style is null or url_style not in ('u', 'u_at', 'clean', 'clean_at');
alter table public.profiles alter column url_style set default 'u_at';
do $$ begin
